...
<header class="flex justify-between items-center mb-10">
    <div>
        ...
        <p class="text-slate-500 font-medium">Atur partner dan sponsor yang mendukung event-event Anda.</p>
    </div>
...
<div class="mb-6">
    <form action="{{ route('admin.partners.index') }}" method="GET" class="flex gap-4">
        <div class="flex-1 relative">
            ...
            <input type="text" name="q" value="{{ request('q') }}" placeholder="Cari nama partner..." class="w-full pl-12 pr-4 py-3 rounded-2xl border border-slate-200 outline-none focus:ring-2 focus:ring-indigo-500 bg-white shadow-sm transition">
        </div>
        ...
        @if(request('q'))
            <a href="{{ route('admin.partners.index') }}" class="px-6 py-3 bg-slate-200 text-slate-700 rounded-2xl font-bold hover:bg-slate-300 transition">Reset</a>
        @endif
    </form>
</div>
    ...
    <div class="overflow-x-auto">
        <table class="w-full text-left border-collapse">
            ...
            <tbody class="divide-y border-t">
                ...
                @empty
                <tr>
                    <td colspan="5" class="px-8 py-12 text-center text-slate-400 font-medium">Tidak ada data partner ditemukan.</td>
                </tr>
                @endforelse
            </tbody>
        </table>
