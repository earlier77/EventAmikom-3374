...
<header class="mb-10">
    ...
    <p class="text-slate-500 font-medium">Ubah nama atau logo partner pendukung event.</p>
</header>
...
<div class="bg-white rounded-[2.5rem] border border-slate-100 p-10 shadow-sm max-w-2xl">
    <form action="{{ route('admin.partners.update', $partner->id) }}" method="POST" enctype="multipart/form-data" class="space-y-6">
        ...
        <div>
            ...
            <input type="text" name="name" value="{{ old('name', $partner->name) }}" class="w-full px-5 py-3 rounded-xl border @error('name') border-rose-500 focus:ring-rose-500 @else border-slate-200 focus:ring-indigo-500 @enderror outline-none focus:ring-2 bg-white transition" required>
            @error('name')
